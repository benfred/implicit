// Faiss doesn't include block select implementations on half data
// since https://github.com/facebookresearch/faiss/pull/1817
// so we're including here
...
void runBlockSelect(Tensor<half, 2, true> &in, Tensor<half, 2, true> &outK,
  ...
  if (dir) {
    ...
    throw std::invalid_argument("unhandled dir");
  }
}

void runBlockSelectPair(Tensor<half, 2, true> &inK, Tensor<int, 2, true> &inV,
                        Tensor<half, 2, true> &outK, Tensor<int, 2, true> &outV,
